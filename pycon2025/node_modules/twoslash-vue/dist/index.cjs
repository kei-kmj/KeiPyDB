...
function createTwoslasher(createOptions = {}) {
  ...
  function getVueLanguage(compilerOptions, vueCompilerOptions) {
    ...
    function getLanguage() {
      const vueOptions = {
        ...languageCore.createParsedCommandLineByJson(ts__default, ts__default.sys, ts__default.sys.getCurrentDirectory(), {}).vueOptions,
        ...vueCompilerOptions
      };
      const vueLanguagePlugin = languageCore.createVueLanguagePlugin(ts__default, twoslash.defaultCompilerOptions, vueOptions, (id) => id);
  ...
  function twoslasher(code, extension, options = {}) {
    ...
    const sourceScript = lang.scripts.set("index.vue", ts__default.ScriptSnapshot.fromString(strippedCode));
