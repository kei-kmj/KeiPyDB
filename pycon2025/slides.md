...
今回利用するSELECT文です。
SELECT 〜〜
で、id = 258, name = 'Alice'というレコードを取得します。
...
トークンの種類を
〜〜のように判定して分類します。
英単語を名詞、動詞、形容詞などに分類するのと同じイメージです
...
```python {all|1-5|1,7-13|1,7,10|all}
class Lexer:
...
<!-- スピーカーノート：
次に構文解析します。
SQL文は人間には理解できても、そのままではコンピュタにとってはただの文字列の羅列です。
そこで、Lexerでトークン列に分解した上で、
Parserでコンピュータが理解できるように構造化する必要があります。
構造化したものがAST(abstract〜抽象構文木)と呼ばれるもので、木構造になっていて、
コンピュータにSQLの構造を伝えることができるようになります
...
まず、現在のトークンがSELECTであることを認識すると、次のトークンの処理に進みます。
...
全てのフィールド名をフィールドリストとして返すことができます。
...
<!-- スピーカーノート：
このtable_list()メソッドでも、最初のテーブル名を認識してリストに追加して、
カンマが続く限り、カンマを認識して次のテーブル名をリストに追加して、
最後にテーブル名のリストを返します。
-->
...
<!-- スピーカーノート：
次に、WHERE句を認識して、次のトークンの処理に進んで、条件式を作成します。
...
<!-- スピーカーノート：
predicateメソッドで条件式を作成するのですが、まずはtermメソッドが呼ばれます。
...
<div v-click="[3, 4]" class="absolute top-[150px] left-[450px]">
  <Balloon text="イコールを認識して次のトークンに進む" />
</div>
...
term()メソッドで、左辺を解析しています。
expressionメソッドで、識別子かリテラルかを判定しています。

でイコールを認識して、
右辺も識別子かリテラルかを判定して、条件式を形成して返します。
...
```
<div v-click="[1, 2]" class="absolute top-[240px] left-[350px]">
...
<img src="/tea.png" style="position: absolute; bottom: 30px; right: 50px; width: 180px; height: auto;" />
...
<!-- スピーカーノート：
スロットのサイズがわかりましたし、中のレイアウトもわかリました。1スロットが22バイトなので、22バイトずつ探していけばいいわけです。
1スロット毎に、13バイト目から10バイト分を見て、nameがAliceかどうかを確認します。
-->
...
スロットのレイアウトの通りにデータが保存されていることがわかります。
...
## <span style="color: #3b82f6;">[https://github.com/kei-kmj/KeiPyDB](https://github.com/kei-kmj/KeiPyDB)</span>
