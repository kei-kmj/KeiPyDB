...
def test_metadata_manager_error_handling(real_metadata_manager_env):
    ...
    # 存在しないテーブルの操作
    assert metadata_manager.get_layout("nonexistent_table", tx) is None
    assert metadata_manager.get_view_definition("nonexistent_view", tx) is None
    assert metadata_manager.get_stat_info("nonexistent_table", tx) is None
...
def test_metadata_manager_stress_test(real_metadata_manager_env):
    """MetadataManagerのストレステスト"""
    file_manager, log_manager, buffer_manager = real_metadata_manager_env
    tx = Transaction(file_manager, log_manager, buffer_manager)
    
    metadata_manager = MetadataManager(True, tx)
    
    # 大量のテーブル、ビュー、インデックスを作成
    num_objects = 10
    ...
    for i in range(num_objects):
        schema = Schema()
        schema.add_field("id", FieldType.Integer, 0)
        schema.add_field(f"field_{i}", FieldType.Varchar, 20)
        ...
        view_def = f"SELECT * FROM {table_name} WHERE id > {i}"
        metadata_manager.create_view(view_name, view_def, tx)
        
        # インデックスを作成（エラーは無視）
        try:
            metadata_manager.create_index(f"stress_idx_{i}", table_name, "id", tx)
        except Exception:
            pass
    ...
    for i in range(num_objects):
        # テーブル確認
        if metadata_manager.get_layout(f"stress_table_{i}", tx) is not None:
            successful_tables += 1
        
        # ビュー確認
        if metadata_manager.get_view_definition(f"stress_view_{i}", tx) is not None:
            successful_views += 1
        
        # インデックス確認
        try:
            index_info = metadata_manager.get_index_info(f"stress_table_{i}", tx)
            total_indexes += len(index_info)
        except Exception:
            pass
    
    print(f"Stress test results:")
    print(f"  Tables: {successful_tables}/{num_objects}")
    print(f"  Views: {successful_views}/{num_objects}")
    print(f"  Total indexes: {total_indexes}")
    
    # 大部分が成功していることを確認
    assert successful_tables >= num_objects * 0.9
    assert successful_views >= num_objects * 0.9
