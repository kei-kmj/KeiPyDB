...
def test_stat_manager_initialization(real_stat_env):
    ...
    assert len(stat_manager.table_stats) == 0
...
def test_statistics_refresh_after_100_calls(real_stat_env):
    """100回の呼び出し後の統計情報リフレッシュテスト"""
    file_manager, log_manager, buffer_manager = real_stat_env
    tx = Transaction(file_manager, log_manager, buffer_manager)
    ...
    # テストテーブルを作成
    table_name = "refresh_test_table"
    create_test_table_with_data(table_manager, tx, table_name, 10)
    ...
    # リフレッシュが発生していることを確認
    assert stat_manager.num_calls >= 100
...
def test_stat_manager_thread_safety(real_stat_env):
    ...
    table_manager = TableManager(True, setup_tx)
    stat_manager = StatManager(table_manager, setup_tx)
    
    create_test_table_with_data(table_manager, setup_tx, "thread_test_table", 25)
    setup_tx.commit()
    ...
    def worker_thread(worker_id):
        try:
            ...
            # 統計情報を複数回取得
            for _ in range(10):
                stat_info = stat_manager.get_stat_info("thread_test_table", tx)
                if stat_info:
