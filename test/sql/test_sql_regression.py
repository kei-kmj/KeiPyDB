...
class TestSQLRegression:
    ...
    def test_create_table_regression(self, fresh_database):
        ...
        result = planner.execute_update("CREATE TABLE regression_test (id varchar(10), data varchar(100))", tx)
        tx.commit()
    ...
    def test_string_handling_regression(self, fresh_database):
        ...
        tx = db.new_transaction()
        planner.execute_update("CREATE TABLE string_test (content varchar(100))", tx)
        planner.execute_update("INSERT INTO string_test (content) VALUES ('Hello World')", tx)
        tx.commit()

        tx2 = db.new_transaction()
        plan = planner.create_query_plan("SELECT content FROM string_test", tx2)
        scan = plan.open()

        scan.before_first()
        if scan.next():
            text_value = scan.get_string("content")
...
def test_comprehensive_sql_workflow(fresh_database):
    ...
    # 2. 初期データ投入
    tx = db.new_transaction()
    initial_data = [("1", "Task1", "pending"), ("2", "Task2", "pending"), ("3", "Task3", "completed")]

    for id_val, name, status in initial_data:
        planner.execute_update(
            f"INSERT INTO workflow_test (id, name, status) VALUES ('{id_val}', '{name}', '{status}')", tx
        )
    tx.commit()
    ...
    scan.before_first()
    while scan.next():
        final_records.append({"id": scan.get_string("id"), "status": scan.get_string("status")})
