...
def test_record_id_hash_consistency():
    # 同じパラメータのRecordIDは等しいが、ハッシュは定義されていない
    rid1 = RecordID(10, 5)
    rid2 = RecordID(10, 5)
    
    # ハッシュが定義されていない場合、オブジェクトのidベースのハッシュになる
    # そのため、異なるオブジェクトは異なるハッシュを持つ
    assert hash(rid1) != hash(rid2)
    
    # 辞書やセットでの使用には注意が必要
    record_set = {rid1, rid2}
    assert len(record_set) == 2  # 同じ値でも異なるオブジェクト
